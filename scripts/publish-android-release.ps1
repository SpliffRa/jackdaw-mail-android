...
try {
    ...
    $releaseNotes = @"
## Что нового в ${Tag}:
- 🔊 **Регулятор громкости уведомлений в Настройках**:
  - Добавлен удобный ползунок управления громкостью уведомлений в разделе «Уведомления и звуки» (от 5% до 100% с шагом и индикацией процентов).
  - При регулировке уровня сразу воспроизводится тестовый тихий «чпок» с установленной громкостью для мгновенной комфортной подстройки под пользователя.
  - Настройка громкости сохраняется в постоянной памяти приложения и управляет громкостью всех звуковых эффектов (входящие письма, подтверждение отправки, SLA оповещения).
- ⏱️ **Умный статус ответа SLA**:
  - Закреплен статус «✓ Ответ дан вовремя» / «✓ Ответ дан с опозданием» при отправке ответа на входящее письмо.
"@
    $payload = @{
        tag_name = $Tag
