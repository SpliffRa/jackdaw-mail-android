...
try {
    ...
    $releaseNotes = @"
## Что нового в ${Tag}:
- 🔕 **Бесшумная синхронизация**:
  - Полностью отключены звуковые сигналы и вибрация во время фоновой и ручной синхронизации.
  - Звуковые уведомления о новых входящих письмах сохранены и работают в штатном режиме.
- 🧹 **Очистка уведомлений синхронизации**:
  - Убрана подпись со счётчиком загруженных писем («Получено: N писем»).
  - Пустые уведомления о синхронизации скрываются целиком без пустых плашек.
"@
