...
try {
    ...
    $releaseNotes = @"
## Что нового в ${Tag}:
- 📋 **Выбор и динамическая подгрузка шаблонов подписи Outlook**:
  - Реализован полноценный каталог шаблонов подписи Microsoft Outlook:
    1. **Outlook для Android**: классическая строка «*Отправлено из Outlook для Android*».
    2. **Деловой Outlook**: «*С уважением, {name}, {email}*».
    3. **Корпоративный Outlook**: полная подпись с именем, почтой и приложением.
    4. **Краткий шаблон**: компактная запись «*{name} • {email}*».
  - В окне ответа и создания письма добавлена кнопка **«Шаблоны подписи»** и меню быстрой подгрузки любого шаблона в 1 клик прямо в текст письма.
  - Подпись аккуратно подгружается в тело ответа над разделителем `-----Исходное сообщение-----`.
  - В Настройках добавлены удобные чипы быстрого выбора шаблона и интерактивные теги `{name}`, `{email}`, `{app}` для кастомизации.
- ⚡ **Устранение задержки инициализации**:
  - Корректная синхронизация и гарантированная загрузка тела ответа и шаблона подписи при открытии письма.
"@
    $payload = @{
        tag_name = $Tag
