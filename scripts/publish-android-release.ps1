...
try {
    ...
    $releaseNotes = @"
## Что нового в ${Tag}:
- 📂 **Исправление скролла и упорядочивания папок**:
  - Полностью устранены конфликты жестов: список папок в режиме «Упорядочить» теперь свободно и плавно скроллится свайпом по всей строке.
  - Устранено наложение элементов при перетаскивании благодаря динамическому расчету высоты строк и стабильным ключам `key(folder.id)`.
  - Запись порядка папок в базу данных отложена до завершения жеста (`onDragEnd`), исключая дергания и микрофризы во время драга.
- 🔍 **Диагностика подключения и синхронизации**:
  - В раздел «Синхронизация и SLA» добавлена панель «ДИАГНОСТИКА ПОДКЛЮЧЕНИЯ» со статусом сессии, кодом ответа сервера и ошибками.
  - Добавлены кнопки немедленной проверки связи («Проверить») и быстрого копирования диагностического отчета в буфер обмена («Отчет»).
  - Исправлено ложное сообщение «Все папки актуальны» при реальных ошибках авторизации или неподдерживаемом протоколе.
"@
    $payload = @{
        tag_name = $Tag
