...
try {
    ...
    $releaseNotes = @"
## Что нового в ${Tag}:
- ✍️ **Подпись в ответах и шаблон по стандарту Microsoft Outlook**:
  - При ответе на письмо автоматически формируется каноничный макет Outlook: в начале письма место для ответа пользователя, далее личная подпись, а следом — цитирование с разделителем `-----Исходное сообщение-----`, датой, отправителем и исходным текстом.
  - В Настройки добавлен раздел **«ПОДПИСЬ ПИСЕМ (ШАБЛОН OUTLOOK)»**: полноценный редактор подписи для аккаунтов, тумблер включения/выключения и кнопка мгновенного сброса к классическому корпоративному шаблону Outlook.
  - На экране создания/ответа добавлена кнопка быстрой вставки подписи.
- 🔊 **Регулятор громкости уведомлений**:
  - Интерактивный ползунок в Настройках для точной регулировки громкости тихого «чпока» с живым предпрослушиванием.
"@
    $payload = @{
        tag_name = $Tag
