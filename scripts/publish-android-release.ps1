...
try {
    ...
    $releaseNotes = @"
## Что нового в ${Tag}:
- 🔄 **Хронология цепочки переписки (от нового к старому)**:
  - В истории цепочки писем порядок изменен на обратный хронологический (`timestamp DESC`): самые свежие ответы всегда находятся вверху цепочки.
- ✍️ **Точный шаблон подписи без лишних меню**:
  - Убраны лишние всплывающие меню и переключатели шаблонов при создании письма. В Настройках задается точный шаблон подписи, который автоматически подгружается внизу сообщений.
- 📁 **Структурированные подпапки в Настройках**:
  - Большой список настроек разделен на аккуратные тематические категории с превью и отдельными экранами:
    1. **Тема оформления** (Тёмная, Светлая, Системная)
    2. **Учетные записи** (Подключенные ящики и протоколы)
    3. **Синхронизация и SLA** (Фоновая синхронизация, Wi-Fi, SLA 30 мин)
    4. **Уведомления и звуки** (Тихий чпок, регулятор громкости, звуки)
    5. **Шаблон подписи** (Точный текст подписи для сообщений)
    6. **Пометка прочитанных (Outlook)** (Сразу, с задержкой, после ответа, вручную)
    7. **О приложении и обновления** (Версия, проверка и скачивание обновлений)
"@
    $payload = @{
        tag_name = $Tag
