...
try {
    ...
    $releaseNotes = @"
## Что нового в ${Tag}:
- 🚫 **Отказ от громоздкого дашборд-мониторинга**: Полностью удалены тяжеловесные панели мониторинга и служебные дашборды. Теперь в приложении только чистый Outlook-стиль списка писем и стандартные папки.
- ⏱️ **Лаконичное время до конца ответа (SLA 30 минут)**:
  - Под каждым входящим письмом в списке отображается компактный таймер: `До ответа: X мин` (строго в рамках 30-минутного регламента) либо `Ответ просрочен`.
  - Внутри письма под текстом показывается аккуратная информационная строка `Ответить в течение: X мин (регламент 30 мин)` без громоздких блоков.
- 🔇 **Устранены перебивания звуков**:
  - Системные каналы уведомлений Android переведены в режим абсолютной тишины (`setSilent(true)`), очищены кэшированные старые каналы.
  - Мягкий акустический «чпок» воспроизводится с аппаратным дебаунсом, больше не накладываясь на стандартные рингтоны Android.
- ✉️ **Главная кнопка — «Ответить всем»**:
  - В деталях письма основной янтарной кнопкой с заливкой и акцентом стала **«Ответить всем»** (Re All), чтобы не потерять участников переписки.
  - Кнопка ответа только автору аккуратно оформлена второстепенным контуром.
"@
    $payload = @{
        tag_name = $Tag
