...
try {
    ...
    $releaseNotes = @"
## Что нового в ${Tag}:
- 🔴 **Счетчик непрочитанных писем на ярлыке приложения (Launcher Badge)**:
  - Добавлено отображение цифры непрочитанных писем на ярлыке приложения на рабочем столе: `1` при одном непрочитанном письме, `2`, `3` и т.д. при большем количестве.
  - Полная поддержка как чистого Android (Android 8.0–15 через Notification Badging `setNumber`), так и фирменных лаунчеров: Samsung One UI, Xiaomi MIUI / HyperOS, Huawei / Honor EMUI, Sony Xperia, Vivo, Oppo ColorOS.
  - Реактивное мгновенное обновление счетчика в реальном времени при открытии/прочтении писем, их архивации, удалении, а также при фоновой синхронизации `MailSyncWorker`.
  - Автоматическая очистка бейджа с ярлыка при отсутствии непрочитанных сообщений (`0`).
- 🌐 **Полноценная авторизация OWA (Outlook Web App)**:
  - Интерактивный веб-вход через WebView для SSO / ADFS / SAML / 2FA / MFA с автоперехватом сессионных cookie и токена `X-OWA-CANARY`.
  - Поддержка прямого Form-Based Auth (FBA) входа по логину и паролю.
- ✍️ **Корректное склонение числительных (русская грамматика)**:
  - Внедрен модуль `PluralRules`: правильные формы `1 письмо`, `23 письма`, `10 писем`, `1 непрочитанное`, `4 непрочитанных`.
"@
    $payload = @{
        tag_name = $Tag
