...
try {
    ...
    $releaseNotes = @"
## Что нового в ${Tag}:
- 🔒 **Полное удаление демонстрационных и тестовых данных почты**:
  - Полностью удалены все синтетические/демо-аккаунты (`president@jackdaw.app`, `Mr. President`, `tech.lead@fastmail.com`) и встроенные тестовые письма.
  - Автоматическая очистка устаревших тестовых записей из локальной базы данных при первом запуске обновления.
- 🚀 **Новый экран первоначальной настройки (First Launch Setup)**:
  - Если в приложении нет аккаунтов, открывается чистый и удобный экран добавления почты.
  - Пользователь полностью сам вводит свои учетные данные: выбор протокола (OWA, Exchange ActiveSync, IMAP/SMTP), отображаемое имя, email, сервер и пароль.
  - Поддержка интерактивной OWA веб-авторизации через SSO / ADFS / SAML / 2FA / MFA, а также прямого подключения.
- 🔴 **Счетчик непрочитанных писем на ярлыке приложения (Launcher Badge)**:
  - Отображение числа непрочитанных сообщений на значке приложения на рабочем столе (Android 8.0-15, One UI, MIUI/HyperOS, EMUI).
"@
    $payload = @{
        tag_name = $Tag
