...
try {
    ...
    $releaseNotes = @"
## Что нового в ${Tag}:
- 🔄 **Полная двусторонняя синхронизация с Exchange / OWA**:
  - Все действия в мобильном клиенте (пометка прочитанным/непрочитанным, удаление в корзину, архивация, пометка важности/звездочкой, восстановление) мгновенно и надежно синхронизируются с сервером Exchange через нативные вызовы UpdateItem, DeleteItem и MoveItem.
  - Почтовый ящик на компьютере (Outlook / веб OWA) и на телефоне теперь синхронизируются в реальном времени.
- 🔔 **Уведомления: строго одно разворачиваемое уведомление**:
  - Устранено дублирование уведомлений: теперь в шторке Android отображается строго одно уведомление.
  - Поддержка разворачивания (InboxStyle / BigTextStyle): для одного письма виден отправитель, тема и начало текста; для нескольких писем — список новых сообщений.
  - Заглушенные папки (Mute): если для папки включен Mute, письма из неё не отображаются в уведомлениях и не издают звуковой сигнал.
- 📄 **Полное отображение писем и кликабельные ссылки**:
  - Устранено обрезание длинных HTML-писем (Jira, системы уведомлений и рассылки): полный 12KB+ HTML загружается через GetItem при открытии письма.
  - Ссылки в письмах четко выделены синим цветом и подчеркиванием.
  - При клике на любую ссылку в письме происходит мгновенный редирект в системный браузер (ссылки не открываются внутри почтового клиента).
  - Поддержка перехода к конкретному письму при клике по уведомлению.
"@
