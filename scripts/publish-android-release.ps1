...
try {
    ...
    $releaseNotes = @"
## Что нового в ${Tag}:
- ⏱️ **Умный статус SLA при ответе на письмо**:
  - При отправке ответа на входящее письмо таймер SLA больше не продолжает тикать или ошибочно показывать «просрочено».
  - Карточка письма в общем списке и детальный экран чтения теперь отображают аккуратный статус: **«✓ Ответ дан вовремя»** (элегантный зеленый бейдж) при соблюдении 30-минутного регламента, либо **«✓ Ответ дан с опозданием»**, если ответ был отправлен после дедлайна.
  - Отвеченные письма автоматически исключаются из активного мониторинга в папке SLA.
- 🔔 **Аутентичный тихий звук уведомлений («Чпок» с эффектом эхо)**:
  - Интегрирован оригинальный звуковой файл `chpok.mp3` (трек с эффектом эхо с сайта zvukipro.com).
  - Воспроизведение звука переведено на высокопроизводительный `SoundPool` с резервным fallback на `MediaPlayer` для мгновенного отклика без задержек и артефактов.
  - Канал уведомлений обновлен до v6 с полным отключением стандартного системного звука Android OS (`setSound(null, null)`), гарантируя чистое звучание без перебиваний.
"@
    $payload = @{
        tag_name = $Tag
