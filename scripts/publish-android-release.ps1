...
try {
    ...
    $releaseNotes = @"
## Что нового в ${Tag}:
- 🗑️ **Безвозвратное удаление в Корзине**: В папке «Корзина» свайп влево и кнопка удаления вызывают действие «Удалить навсегда». Письмо и все вложения физически удаляются из базы SQLite безвозвратно.
- 📖 **Удобное чтение писем (SLA под телом)**: В детальном просмотре входящих писем блок SLA перенесен под текст письма. Сверху сразу видны тема, отправитель и сам текст, как в отправленных.
- ☀️ **Контрастность SLA в светлой теме**: Все бейджи и карточка SLA автоматически переключаются на высококонтрастные пастельные подложки с четким темным текстом (#047857, #B45309, #B91C1C) на светлом фоне.
- 🛡️ **Продуманная безопасность**: Диалог подтверждения четко предупреждает о безвозвратном удалении в корзине, исключая случайную потерю данных.
"@
    $payload = @{
        tag_name = $Tag
