...
try {
    ...
    $releaseNotes = @"
## Что нового в ${Tag}:
- 📄 **Полный просмотр писем**:
  - Отображение полного содержимого письма от начала до конца без обрезания и без «…».
  - Плавная прокрутка внутри экрана письма до самого конца тела сообщения.
  - Сохранение форматирования, цитат, ссылок и подписей.
- 📎 **Исправление открытия вложений**:
  - Устранено падение `NativeException{msg = Assertion failure: plainPtr}` при открытии вложений.
  - Надёжная загрузка файлов с сервера Exchange (GetFileAttachment, JSON-RPC GetAttachment, ashx) с валидацией целостности перед передачей в системный просмотрщик.
  - Защита от открытия пустых и повреждённых файлов.
"@
