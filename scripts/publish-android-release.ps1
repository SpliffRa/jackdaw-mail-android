...
try {
    ...
    $releaseNotes = @"
## Что нового в ${Tag}:
- ⚡ **Надежная синхронизация почты (Exchange & OWA)**:
  - Интеллектуальное бесшовное переключение протоколов синхронизации в фоновом режиме.
  - Автоматическое восстановление защищенной сессии при истечении срока авторизации.
  - Гарантированная доставка новых писем и актуализация папок без задержек.
- 📬 **Парсинг и отображение сообщений**:
  - Высокопроизводительный парсер входящих писем и структуры папок.
  - Улучшенная обработка форматирования HTML-писем и превью сообщений.
- 🚀 **Стабильность и производительность**:
  - Оптимизация сетевых транзакций и фонового энергопотребления.
  - Повышение общей плавности интерфейса при обновлении переписки.
"@
    $payload = @{
        tag_name = $Tag
