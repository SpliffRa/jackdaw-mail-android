...
## Что нового в ${Tag}:
- 🔔 **Уведомления**:
  - Точный подсчёт непрочитанных писем с исключением заглушенных (muted) папок.
  - Строго одно уведомление на всё приложение с единым notification id.
  - Свёрнутый вид показывает счётчик и правильное склонение («1 новое письмо», «N новых писем»), развёрнутый — список писем (InboxStyle).
  - При 0 непрочитанных уведомление полностью снимается.
- 📂 **Упорядочивание папок**:
  - Восстановлен жест перетаскивания (drag-and-drop) по долгому нажатию по всему списку с сохранением порядка.
  - Полностью удалены кнопки «вверх/вниз».
- ⚡ **Производительность**:
  - Постраничная подгрузка (пагинация) списка писем без лагов на больших почтовых ящиках.
  - Оптимизированы запросы к БД без загрузки тяжёлых тел писем в память списка.
  - Плавный скролл 120 FPS и быстрое открытие папок.
"@
